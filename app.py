...
# 🥠 擴充至 23 種外送專屬運勢
FORTUNE_POOL = [
    "🌟 大吉：天地靈氣匯聚！一路綠燈，無縫接單，連下暴雨都能遇到天使客人給小費！",
    "🌟 大吉：紫氣東來！今天接到的都是近距離高單價，宛如仙人指路，爽賺一波！",
    "🌟 大吉：財神庇佑！小費拿到手軟，系統瘋狂派單，破紀錄就在今天！",
    "🌟 大吉：萬物皆明！客人剛好都在一樓等你，地址清晰好找，如同天助！",
    "✨ 中吉：運勢平穩。單量穩定，穩紮穩打，是個累積修為的好日子。",
    "✨ 中吉：靈氣溫和。避開了雷包店家，取餐順利，行車平安。",
    "✨ 中吉：微風扶柳。雖然沒有爆單，但單單順路，油耗極佳，堪稱節能天尊！",
    "✨ 中吉：遇貴人相助。迷路時剛好有同行指點，順利抵達目的地。",
    "🌤️ 小吉：略有波折。轉角可能會遇到三寶，請保持防禦駕駛，阿彌陀佛。",
    "🌤️ 小吉：福禍相依。遇到要爬五樓的客人，當作鍛鍊體魄，修為穩步提升！",
    "🌤️ 小吉：靈氣波動。店家拖餐讓你多等了十分鐘，但剛好可以趁機回個訊息。",
    "🌤️ 小吉：偶遇陣雨。記得穿好雨衣，水滴是上天賜予的試煉！",
    "🍃 平：普普通通的一天。保持平常心，心若冰清，天塌不驚。",
    "🍃 平：天地無極，乾坤借法。今天沒什麼特別的，早點回家洗洗睡也是一種修行。",
    "🍃 平：風平浪靜。沒有奧客也沒有小費，佛系跑單，隨遇而安。",
    "💧 末吉：接到大單，但絕對是六樓沒有電梯，還要扛三桶水！當作重訓吧！",
    "💧 末吉：陰雨綿綿。導航可能會帶你走進死胡同，請相信自己的直覺。",
    "💧 末吉：靈氣枯竭。網路訊號可能不太穩，記得隨時重開 APP，以免錯失仙緣。",
    "⚠️ 凶：暗藏殺機！防禦駕駛，避開雷包店家，今天極易遇到湯汁外漏的劫數！",
    "⚠️ 凶：天狗食月！注意你的輪胎！今天出門前務必檢查胎壓，不然準備牽車顧路！",
    "⚠️ 凶：煞氣沖天！準備迎接奧客的靈魂拷問，請深呼吸，不要跟凡人計較！",
    "💀 大凶：天降神罰！手機架可能斷裂，或者遇到「現金單不給錢」魔物，建議在家閉關！",
...
<style>
    ...
    .vip-box { border: 1px dashed #06C167; padding: 10px; border-radius: 8px; background-color: rgba(6, 193, 103, 0.1); margin-top: 10px; font-size: 14px; margin-bottom: 20px;}
...
# ==========================================
# 分頁 0: 🐉 宗門大殿 (修正渲染衝突，乾淨版面)
# ==========================================
with tab0:
    st.markdown("### 📜 個人狀態面板")
    
    # 🌟 第一區塊：狀態列 (移除包覆按鈕的 HTML，僅用於純文字顯示)
    ...
    with stat_c3:
        # 進度條使用原生寫法，不包覆在自訂 HTML 中，避免破版空白
        st.write("##### ⚡ 靈石突破進度")
        st.progress(prog)
        st.caption(f"目前：{total_stone:,} / 需求：{n_exp:,}")

    if atk_tier > 0:
        with st.expander(f"👑 展開你的 VIP {atk_tier} 專屬境界特權"):
            st.markdown(f"""
            - 📈 **被動光環**：每日結算總戰力獲得 **{cp_mult} 倍**加成！
            - 🎲 **煉丹保底**：大暴擊機率增加 **{alc_win_buff}%**，炸爐機率減少 **{alc_rug_red}%**。
            - 🛡️ **護體罡氣**：遭暗算時，傷害減免 **{int((1-def_mult)*100)}%**，有 **{def_rebound}%** 機率反彈傷害！
            """)

    st.divider()

    # 🌟 第二區塊：宗門大亂鬥 (使用原生排版，徹底解決空白問題)
    ...
    with play_c1:
        ...
        st.subheader("🎲 激進煉丹爐")
        st.info(f"💡 投入 50 點戰力。成功率：**{base_win}%** / 炸爐率：**{base_rug}%**")
    ...
    with play_c2:
        ...
        if not friends_dict:
            ...
            with opt_c1: attack_type = st.radio("手段", ["📌 圖釘(-30 CP)", "🧛 吸星(-80 CP)"])
            with opt_c2: stealth_mode = st.radio("身分", ["⚔️ 實名(必中)", "🥷 匿名(50%失敗)"])
